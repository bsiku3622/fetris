...
/**
 * MARATHON 레벨별 중력 — min(20, 0.02 * 1.25^lvl)를 lvl 0..30에 대해 사전 계산.
 * lvl 31부터는 상한 20에 걸린다. Math.pow는 엔진 구현 재량이라 리터럴로 고정한다
 * (결정론 배경은 scoring.ts 헤더 주석 참고).
 */
const MARATHON_GRAVITY: readonly number[] = [
  0.02, 0.025, 0.03125, 0.0390625,
  0.048828125, 0.06103515625, 0.0762939453125, 0.095367431640625,
  0.11920928955078125, 0.14901161193847656, 0.1862645149230957, 0.23283064365386963,
  0.29103830456733704, 0.3637978807091713, 0.4547473508864641, 0.5684341886080801,
  0.7105427357601002, 0.8881784197001252, 1.1102230246251565, 1.3877787807814457,
  1.734723475976807, 2.168404344971009, 2.710505431213761, 3.3881317890172014,
  4.235164736271502, 5.293955920339378, 6.617444900424222, 8.271806125530277,
  10.339757656912846, 12.924697071141056, 16.15587133892632,
];
...
export class Mode {
  ...
  update(game: Game, _dtFrames: number): void {
    ...
    if (this.name === "marathon") {
      // 라인 수에 따라 점진적 중력 상승 — 테이블 범위를 넘으면 상한 20
      const lvl = Math.floor(game.stats.lines / 10);
      game.gravityOverride = lvl < MARATHON_GRAVITY.length ? MARATHON_GRAVITY[lvl] : 20;
    }
  }
