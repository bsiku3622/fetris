...
봇이 "이렇게 놓을 생각"을 반투명 고스트로 자기 보드에 띄울 수 있습니다. AI가 가방 단위로 세운 계획을 보여주는 용도입니다.

```jsonc
{ "t": "relay", "msg": { "t": "plan", "ghosts": [
  { "piece": 6, "rot": 0, "x": 3, "y": 38, "alpha": 0.55 }
] } }
```

- `piece`/`rot`은 엔진의 `Piece`·`Rot` 값, `x`/`y`는 보드 좌표(`y`는 버퍼 포함 — `Game.py`와 같은 계)입니다.
- `alpha`는 선택. 뒤쪽 계획을 흐리게 하는 데 씁니다.
- **`ghosts: []`를 보내면 지워집니다.** 판이 끝날 때 걷어주는 게 좋습니다.
- 한 번에 `MAX_PLAN_GHOSTS`(32)개까지만 그려집니다 — 보드를 통째로 덧칠하는 걸 막습니다.

**게임 상태가 아닙니다.** 시뮬레이션·판정·리플레이 검증 어디에도 끼지 않고 화면에만 그려지므로 결정론을 건드리지 않습니다. 서버는 이 메시지를 해석하지 않고 중계하지만 녹화에는 담기 때문에, 관전자와 나중에 리플레이를 보는 사람에게도 그 시점에 떠 있던 계획이 그대로 재생됩니다.

봇도 판이 끝나면 사람과 똑같이 `replay`를 제출하는 것이 좋습니다. 서버가 그 제출을 방에 흘려주므로 따로 나눠줄 필요는 없습니다. 관전자는 자기 입력 로그가 없어 **참가자가 낸 기록으로만** 그 경기를 내려받을 수 있으므로, 봇만 뛰는 방에서 이걸 빼먹으면 구경한 사람에게 아무것도 남지 않습니다. 참조 러너(`examples/bot-runner.mjs`)에 구현이 있습니다.
