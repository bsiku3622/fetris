...
제출은 검증만 하고 끝나지 않습니다. 서버는 받은 기록을 `replay-record`로 방에 그대로 흘려주고, 클라이언트가 그것들을 모아 **판 하나짜리 매치 리플레이**로 조립합니다. 그래서 참가자는 한 번만 내면 되고, 자기 로그가 없는 관전자도 그 경기를 통째로 내려받아 모든 보드를 나란히 다시 볼 수 있습니다. 리플레이를 지원하지 않는 봇의 판만 목록에서 빠집니다.
