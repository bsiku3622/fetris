...
| `{ t: "ko" }` | 내가 탈락했다는 자기 신고 |
| `{ t: "replay", matchId, frames, keys, garbage, fingerprint }` | 매치 종료 후 리플레이 제출(검증용) |
...
봇도 판이 끝나면 사람과 똑같이 `replay`를 제출하고 `replay-share`로 방에 기록을 나눠주는 것이 좋습니다. 관전자는 자기 입력 로그가 없어 **참가자가 나눠준 기록으로만** 그 경기를 내려받을 수 있으므로, 봇만 뛰는 방에서 이걸 빼먹으면 구경한 사람에게 아무것도 남지 않습니다. 참조 러너(`examples/bot-runner.mjs`)에 구현이 있습니다.
