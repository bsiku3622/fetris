...
| 메시지 | 설명 |
|---|---|
| `{ t: "create", maxPlayers?, nick?, handling? }` | 방 생성 → `{ t: "created", code, myId, state, session }` |
| `{ t: "join", code, nick?, ticket?, handling? }` | 방 입장 → `{ t: "joined", ... }` / `{ t: "error", reason }` |
| `{ t: "resume", token, lastSeenId? }` | 끊겼던 자리로 복귀 → `{ t: "resumed", ... }` |
| `{ t: "set-role", role }` | 참가자 ↔ 관전자 전환(lobby 전용) |
| `{ t: "config", config }` | (호스트) 매치 설정 갱신 |
| `{ t: "handling", handling }` | 내 감도 알림 — 남들이 내 보드를 따라 돌리는 데 쓴다 |
...
| `{ t: "relay-to", targetId, msg, mid? }` | 특정 플레이어에게만 중계 |
...
서버 → 클라이언트(`ServerControl`): `created`, `joined`, `resumed`, `state`, `match-start`, `ko`, `match-end`, `plan-state`, `replay-record`, `recording`, `error`, `relay`, `bot-ready`, `bot-invite`, `bot-pending`, `runners`. 모든 메시지에 증가하는 `id`가 붙어 복귀 때 어디까지 받았는지 대조합니다.

`match-start`에는 참가자별 `sim`(`{ id, seed, handling }`)이 함께 실립니다. 서로의 보드를 입력만으로 따라 돌리려면 둘 다 있어야 하고, 시드는 서버가 나눠줍니다 — 조각 순서를 공유하지 않는 방이라도 서버가 알아야 검증할 수 있기 때문입니다.

<a id="시작-카운트다운"></a>
**`match-start`는 "판을 열어라"이지 "지금 두어라"가 아닙니다.** 판이 열린 뒤 첫 조각이 내려오기까지 **240프레임(60Hz 기준 4초)** 동안 잠겨 있습니다(`READY_FRAMES`). 이 구간이 화면의 3·2·1 카운트다운이고, 0이 되는 순간이 GO입니다. 참가자는 프레임 0을 `match-start`를 받은 시점에 두고 여기서부터 프레임을 셉니다 — 대기 중에도 프레임은 흐르므로, 기록기의 `commitFrame()`은 첫 프레임부터 매 프레임 호출해야 합니다.

이 길이는 **모두가 같아야 합니다.** 짧게 세면 그만큼 먼저 움직이고, 그보다 나쁜 일이 벌어집니다 — 남들 화면에서 그 사람의 판이 통째로 어긋납니다. 미러는 자기 240프레임을 정직하게 세는 동안 들어온 입력을 흘려보내므로, 그 뒤로는 프레임 기준이 어긋난 채 재생되어 조각이 엉뚱한 자리에 놓입니다. 엔진의 `Game`을 그대로 쓰면 저절로 지켜지고, `Game.readyFrames`는 혼자 하는 판에서만 줄이는 값입니다(대전에서 건드리면 안 됩니다).
...
- 시드는 **서버가 배정한 값**을 씁니다. 예전엔 `sharePieces`가 꺼진 방에서 각자가 자기 시드를 뽑았고, 그러면 서버에 맞춰볼 근거가 없어 검증이 통째로 꺼졌습니다. 지금은 어떤 방이든 검증합니다.
- 재현은 CPU를 쓰므로 이벤트 루프 밖(`setImmediate`)으로 미루고, 프레임·키 길이에 상한을 둡니다.

## 매치 녹화

**판을 다시 보는 근거는 서버가 직접 갖습니다.** 중계하는 김에 두 가지를 받아 적습니다.

- **입력 스트림** — `sync`로 흘러가는 키·가비지 로그를 참가자별로 이어 붙입니다. 숫자 배열을 뒤에 붙일 뿐 그게 무슨 조작인지는 여전히 서버의 관심 밖이지만, 이것만 있으면 그 사람 판을 **60Hz로 정확히 되살릴 수 있습니다** — 본인이 아무것도 제출하지 않아도요.
- **보드 상태** — 입력 릴레이의 키프레임 `full`과, 입력을 흘리지 않는 옛 봇의 `board`. 시간축을 붙여 담아 두고, 입력 로그가 없는 참가자는 이걸로 재생합니다.

참가자 명단은 **판을 시작할 때 박아 둡니다.** 끝나고 다시 훑으면 중간에 나간 사람이 통째로 빠져, 분명히 있었던 판이 기록에는 없는 셈이 됩니다.

내려받기는 `get-recording`으로 요청할 때만 보냅니다(몇 MB가 될 수 있어 자동으로 뿌리지 않습니다). 방마다 상한이 있고, 넘으면 그 지점에서 멈추고 `truncated`로 표시합니다.

검증용으로 따로 제출한 사람은 그 기록을 씁니다 — 지문이 함께 있어 파일을 열 때 온전한지까지 확인할 수 있기 때문입니다. 클라이언트가 이 둘을 한 파일로 합쳐 내려받습니다.

제출은 검증만 하고 끝나지 않습니다. 서버는 받은 기록을 `replay-record`로 방에 흘려주고, 클라이언트가 녹화와 함께 묶습니다.

**감도는 사람마다 다릅니다.** 핸들링은 마우스 감도 같은 개인 설정이라 방이 강제하지 않습니다. 그래서 클라이언트가 앉을 때(`create`/`join`) 또는 `handling` 메시지로 자기 값을 알려주고, 서버가 그걸 보관했다가 매치 시작 때 방 전체에 실어 보냅니다(`match-start`의 `sim`). 재현에도 같은 값을 씁니다 — 전부 정상 범위의 설정값이라 자기 신고로 얻을 이득은 없습니다.
...
**지난 판의 페이로드는 버립니다.** 판이 바뀌는 찰나에는 아직 `match-start`를 못 본 참가자가 지난 판 입력을 흘려보냅니다. 그게 상대의 새 미러에 들어가면 몇 천 프레임을 한꺼번에 앞질러 돌며 보드가 통째로 어긋나므로, 중계 봉투에 붙은 `mid`(몇 번째 판인지)를 지금 판과 대조해 걸러냅니다. 페이로드는 여전히 들여다보지 않습니다 — 봉투의 번호만 봅니다. 번호가 없는 메시지(채팅, 아직 안 고친 봇)는 그대로 통과시킵니다.

**보드가 아니라 입력이 오갑니다.** 클라이언트는 프레임이 찍힌 키 변화와 받은 가비지를 `sync`로 흘려보내고, 받는 쪽이 같은 시드·감도로 다시 돌려 그 사람 판을 재현합니다. 실측으로 8.5초 한 판에 나간 게 `sync` 298개 16KB + 키프레임 9개 11KB였습니다 — 같은 시간을 보드 스냅샷으로 채우면 100KB가 넘습니다. 서버는 여전히 내용을 해석하지 않고 중계만 합니다.

WebSocket 압축(`permessage-deflate`)을 **컨텍스트 유지 상태로** 켭니다. 오가는 메시지가 서로 거의 같은 모양이라, 메시지마다 따로 압축하면 3배 남짓이지만 컨텍스트를 이어가면 같은 자료가 50배 이상 줄어듭니다(실측: 스냅샷 367KB → 소켓 6.8KB). 작은 제어 메시지는 압축 이득보다 오버헤드가 커서 `threshold` 아래로 그냥 흘려보냅니다.

**순단 복귀.** 서버가 보내는 모든 메시지에 증가하는 번호를 붙이고 최근 것을 들고 있습니다. 판이 도는 중에 소켓이 끊기면 자리를 잠깐(15초) 잡아두고, 클라이언트가 세션 토큰으로 `resume`하면 그 사이 빠진 것만 다시 보냅니다. 재전송 버퍼는 제어와 중계를 따로 담습니다 — 한 통에 섞으면 초당 백 개 넘게 흐르는 입력 스트림에 밀려 KO·매치 종료가 통째로 빠집니다.
...
**봇도 시작 카운트다운을 지켜야 합니다.** `match-start`를 받자마자 두기 시작하면 사람 눈에는 봇 혼자 먼저 움직이고, 남들 화면에서는 그 봇의 판이 통째로 어긋나 라인 하나 못 지운 채 쌓여 올라갑니다. 자세한 규약은 위 [시작 카운트다운](#시작-카운트다운)에 있습니다. 봇이 정말 규약대로 도는지는 이렇게 확인합니다.

- **첫 하드드롭이 240프레임 이후인가** — 자기 `ReplayRecorder`의 로그에서 첫 `HardDrop` 항목의 프레임을 보면 됩니다.
- **남들이 보는 판이 내 판과 같은가** — 2초마다 내보내는 `full` 키프레임과, 같은 프레임에서 `BoardMirror`가 스스로 도달한 상태의 지문(`snapshotFingerprint`)이 같아야 합니다. 어긋나면 미러가 그 자리에서 되돌리므로 화면이 2초마다 튑니다.
