...
- `config.rule`을 **그대로** `Game`에 넘기세요. 판을 열 때 입력을 잠가두는 시간(`readyFrames`)처럼 시작 타이밍을 정하는 항목이 들어 있어서, 임의로 만든 룰로 돌리면 봇만 먼저 두기 시작하고 남들 화면의 미러와 판이 갈립니다.
...
- **엔진을 낡은 채로 둔다** → 룰에는 시뮬레이션 타이밍을 바꾸는 항목이 있습니다(판을 열 때 입력을 잠가두는 `readyFrames` 등). 모르는 항목은 그냥 무시되므로 봇만 다른 타이밍으로 돌게 되고, 남들 화면의 미러와 판이 통째로 갈립니다. **엔진을 고친 뒤에는 `npm run build:engine`을 다시 돌리고 러너를 재시작하세요.**
