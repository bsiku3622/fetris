...
- `config.rule`을 **그대로** `Game`에 넘기세요. 임의로 만든 룰로 돌리면 남들 화면의 미러와 판이 갈립니다.
- **출발은 `match-start`가 도착하는 순간입니다.** 받자마자 판을 열면 됩니다 — 엔진이 첫 **240프레임(4초)** 동안 입력을 잠가주므로(`Phase.Ready`) 모두가 같은 시점에 시작합니다. 화면에는 그 사이 3·2·1·GO!가 뜹니다. 따로 세지 마세요.
  - 엔진을 직접 구현했다면 이 대기 프레임을 **똑같이** 두어야 합니다(`READY_FRAMES = 240`). 짧게 두면 그만큼 먼저 움직이고, 남들 화면의 미러와도 판이 갈립니다.
...
- **엔진을 낡은 채로 둔다** → 룰에는 판의 전개를 정하는 항목이 잔뜩 있습니다. 모르는 항목은 그냥 무시되므로 봇만 다른 규칙으로 돌게 됩니다. **엔진을 고친 뒤에는 `npm run build:engine`을 다시 돌리고 러너를 재시작하세요.**
