...
- **보드가 아니라 입력을 흘려보냅니다**(TETR.IO 방식). `sync` 메시지에 프레임이 찍힌 키 변화와 받은 가비지를 실어 보내면, 받는 쪽이 `BoardMirror`(`packages/engine/src/mirror.ts`)로 같은 시드·감도에 같은 입력을 먹여 그 사람 판을 그대로 다시 돌립니다. 스냅샷을 뿌리던 때보다 트래픽이 20배 남짓 적고, 미러도 진짜 게임이라 조각이 실제로 떨어집니다(스핀·B2B 같은 엔진 이벤트도 그대로 나와 관전 사운드가 정확해집니다).
  - 흘려보내는 자료는 리플레이 기록기(`VersusMatch.recorder`)가 쌓는 로그 그 자체입니다. 검증에 낼 기록과 남들이 보던 화면이 같은 곳에서 나오므로 둘이 어긋날 수 없습니다.
  - 미러는 상대가 알려준 `upto` 프레임까지만 진행합니다. 넘어가면 아직 오지 않은 입력을 없는 셈 치고 돌게 되어 어긋납니다.
  - 순단으로 입력이 통째로 빈 구간은 따라 돌 수 없으므로, 낮은 빈도(`KEYFRAME_FRAMES`)로 상태 키프레임(`full`)을 함께 보냅니다. 미러는 크게 벌어졌을 때만 이걸로 되돌립니다 — 매번 갈아끼우면 그때마다 화면이 튑니다.
  - **시드와 감도는 서버가 나눠줍니다**(`match-start`의 `sim`). 감도는 개인 설정이라 사람마다 다르고, 시드는 서버가 알아야 검증할 수 있습니다.
- KO돼도 세션은 살아 있습니다. 로컬 보드만 잠기고 관전으로 넘어가며, 승패 판정은 서버가 합니다.
- 소켓이 끊겨도 판이 도는 중이면 서버가 잠깐 자리를 잡아둡니다. 클라는 세션 토큰으로 `resume`해 같은 자리로 돌아오고, 서버·클라 양쪽이 메시지에 번호를 붙여 그 사이 빠진 것만 다시 주고받습니다.
